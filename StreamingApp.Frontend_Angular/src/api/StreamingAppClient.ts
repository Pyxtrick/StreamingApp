...
export class TwitchClient extends BaseClient {
    ...
    getTwitchChatData(): Promise<void> {
        let url_ = this.baseUrl + "/api/Twitch";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetTwitchChatData(response: Response): Promise<void> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
...
E:\Git\StreamingApp\StreamingApp.Frontend\src\api\BaseClient.ts